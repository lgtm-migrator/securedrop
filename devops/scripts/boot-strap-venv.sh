...
function virtualenv_bootstrap() {
    ...
    then
        ...
        if [ ! -d "$VENV" ]
        then
            p=$(which "python${PYTHON_VERSION}" 2> /dev/null || which python3)
            echo "Creating ${p} virtualenv in ${VENV}"
            virtualenv -p "${p}" "${VENV}"
        fi

        "${VENV}/bin/pip" install -q -r "securedrop/requirements/python${PYTHON_VERSION::1}/develop-requirements.txt"
